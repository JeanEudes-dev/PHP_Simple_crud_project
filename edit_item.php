...
if (isset($_GET['id'])) {
    ...
    if (isset($_SESSION['user_level']) && $_SESSION['user_level'] === 'admin') {
        ...
        $stmt = $conn->prepare("SELECT description, photo FROM Items WHERE id = ?");
        $stmt->bind_param("i", $item_id);
    } else {
        // Regular user can only edit their own items
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['item_id'])) {
    ...
    if (isset($_SESSION['user_level']) && $_SESSION['user_level'] === 'admin') {
        ...
        // Regular user can only update their own items
    ...
    if (!empty($_FILES['photo']['name'])) {
        ...
        if (move_uploaded_file($photo['tmp_name'], $uploadFilePath)) {
            ...
            if (isset($_SESSION['user_level']) && $_SESSION['user_level'] === 'admin') {
                // Admin can update any item
                $stmt = $conn->prepare("UPDATE Items SET description = ?, photo = ? WHERE id = ?");
                $stmt->bind_param("sss", $description, $photo_filename, $item_id);
            } else {
                // Regular user can only update their own items
    ...
    <div class="container mt-5">
        ...
        <?php else: ?>
            <div class="alert alert-danger text-center">
                Item not found or you do not have permission to edit this item.
                <a href="manage_items.php" class="alert-link">Go back to items list</a>
            </div>
        <?php endif; ?>
    </div>
